...
TEST_F(ButtonStateTest, RisingEdge_TrueOnFirstPress) {
    XINPUT_STATE s{}; s.Gamepad.wButtons = XINPUT_GAMEPAD_A; nexPad.setCurrentState(s);
...
TEST_F(ButtonStateTest, FallingEdge_TrueOnRelease) {
    ...
    XINPUT_STATE r{}; nexPad.setCurrentState(r);  // zero wButtons
...
TEST_F(ButtonStateTest, Combo_BothBitsRequired) {
    ...
    XINPUT_STATE s{}; s.Gamepad.wButtons = (WORD)combo; nexPad.setCurrentState(s);
